...
vanilla = pd.read_csv('vanilla_experiments.csv')
vl = len(vanilla.index)
approach1 = pd.read_csv('experiments_approach1.csv')
a1l = len(approach1.index)
approach2 = pd.read_csv('experiments_approach2.csv')
...
for model in v_models:
	avg_vanilla[model] = [vanilla['Mutate_time'].mean(), vanilla['Mutation_Score'].mean(), vanilla['MS_time'].mean()]
...
for model in a1_models:
	...
	for n in [2, 4, 6, 8, 10]:
		temp += [tuple((n,
				   [approach1[approach1['Clusters_per_layer'] == n][approach1['Model_Type'] == model]['Mutate_time'].mean(),
					approach1[approach1['Clusters_per_layer'] == n][approach1['Model_Type'] == model]['Cluster_time'].mean(),
					approach1[approach1['Clusters_per_layer'] == n][approach1['Model_Type'] == model]['Mutation_Score'].mean(),
					approach1[approach1['Clusters_per_layer'] == n][approach1['Model_Type'] == model]['MS_time'].mean()]))]
	avg_a1[model] = temp
...
for key, value in avg_a2.items():
	for thing in value:
		label += [key]
		X += [thing[0]]
		Y += [float(((avg_vanilla[key][0]) - (thing[1][0])) / (avg_vanilla[key][0]))]
		#((vanilla mutation testing time) - (approach-1-2-mutation testing time)) / (vanilla mutation testing time)
...
for label in unique_labels:
    ...
    plt.scatter(X_clean[indices], Y_clean[indices], label=label)
...
# Show plot
plt.savefig('Approach2_MutTimeSpeedup-vs-PARHAC.jpeg')
...
plt.savefig('Approach1_MutTimeSpeedup-vs-Clustersz.jpeg')
